...
def get_red_pajama(nsamples, seqlen, tokenizer, eval_mode=False):
    ...
    print(f"Loading red_pajama from {dataset_repo}")
    assert not eval_mode, "Only train set is supported in RedPajama"
    try:
        traindata = load_dataset(dataset_repo, split="train")
    except RuntimeError as exc:
        if "togethercomputer/RedPajama-Data-1T" in dataset_repo and (
            "Dataset scripts are no longer supported" in str(exc)
            or "RedPajama-Data-1T.py" in str(exc)
        ):
            return _get_red_pajama_from_together_stream(nsamples, seqlen, tokenizer)
        raise
...
def _get_red_pajama_from_together_stream(nsamples, seqlen, tokenizer):
    print("Falling back to direct Together RedPajama shard streaming")
    with urlopen(TOGETHER_REDPAJAMA_URLS, timeout=60) as response:
        urls = [line.strip() for line in response.read().decode("utf-8").splitlines() if line.strip()]
    random.shuffle(urls)
    ...
    raise RuntimeError(f"Together RedPajama fallback produced only {len(trainloader)} / {nsamples} samples")
