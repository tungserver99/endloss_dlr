...
def _get_layer_loader(analyzer, gradients):
    def layer_loader(l):
        layer_module_names = list(analyzer.get_layer_module_paths(l).keys())
        # Convert from torch.bf16 to np.fp32 for numba processing
        # Only converts one layer at a time to avoid excessive memory usage
        gradient_layer = [gradients[l][name].float().numpy() for name in layer_module_names]
        model_layer = [analyzer.get_layer_weights(l)[name].float().numpy() for name in layer_module_names]
        return layer_module_names, gradient_layer, model_layer
    ...
    if pipelined_io:
        with ThreadPoolExecutor(max_workers=io_workers) as io_executor:
            for l in tqdm(layers_to_process, desc="Quantizing layers..."):
                ...
                layer_module_names, gradient_layer, model_layer = future_load.result()
                ...
                io_executor.submit(layer_saver, layer_module_names, luts_by_bit_by_module, parent_weights, l)
            logging.info("Waiting for IO to finish...")
    else:
        for l in tqdm(layers_to_process, desc="Quantizing layers..."):
            layer_module_names, gradient_layer, model_layer = layer_loader(l)
            ...
            layer_saver(layer_module_names, luts_by_bit_by_module, parent_weights, l)
