...
def collect_sqllm_squared_gradients_chunked(
    ...
    """Collect SqueezeLLM squared weight gradients without storing all layer grads on GPU."""
    root = Path(output_folder)
    config = {
        "schema": 1,
        "source": "sqllm_squared_weight_gradient_chunked",
    ...
    if (
        ...
        logging.info("Reusing chunked SqueezeLLM squared-gradient cache at %s", root)
        return
    ...
    for chunk_start, layer_chunk in _iter_layer_chunks(layers, max(1, layer_chunk_size)):
        chunk_paths = [
            root / f"l{chunk_start + local_idx}.pt"
            for local_idx in range(len(layer_chunk))
        ]
        if (
            not overwrite
            and config_path.exists()
        ...
        for start in tqdm(
            ...
            desc=f"SqueezeLLM chunked gradients L{chunk_start}-{chunk_start + len(layer_chunk) - 1}",
            **_progress_kwargs(),
        ):
...
def build_sqllm_q0_from_chunked_gradients(
    ...
    if not overwrite and _q0_complete(output, bits, analyzer.num_layers):
        logging.info("Reusing chunked SqueezeLLM q0 cache at %s", output)
        return

    cpu_count = int(cpu_count or os.cpu_count() or 1)
    numba.set_num_threads(max(1, cpu_count))
    logging.info("Building q0 with original SqueezeLLM quantizer from chunked gradients")
    for layer_idx in tqdm(range(analyzer.num_layers), desc="Building chunked SqueezeLLM q0"):
...
def ensure_chunked_sqllm_initialization(
    ...
    if not overwrite and _q0_complete(output_folder, bits, analyzer.num_layers):
        logging.info("Reusing chunked SqueezeLLM q0 cache at %s", output_folder)
        return
