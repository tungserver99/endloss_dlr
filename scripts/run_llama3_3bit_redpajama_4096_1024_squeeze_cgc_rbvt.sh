...
POST_QUANTIZED_PATH="./cache/post_sqllm_quantized/${MODEL_BASENAME}-w${BITS}-sqllm-cgc-rbvt-${DATASET}_s${NUM_EXAMPLES}_blk${SEQ_LEN}_lambda${RBVT_LAMBDA_TAG}"
TOKENS_PATH="./cache/tokens/${MODEL_BASENAME}-${DATASET}_s${NUM_EXAMPLES}_blk${SEQ_LEN}.pt"
MODEL_PATH="./cache/post_sqllm_packed/anyprec-sqllm-cgc-rbvt-${MODEL_BASENAME}-w${BITS}-${DATASET}_s${NUM_EXAMPLES}_blk${SEQ_LEN}_lambda${RBVT_LAMBDA_TAG}"
